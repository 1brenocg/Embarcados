...
unsigned int but = 31, but2 = 29, but3 = 30, but4 = 4;
...
static void DisplayNumber(unsigned int, unsigned int, unsigned int, unsigned int);
...
*******************************************************************************/
    ...
    // gpioIntTypeSet(SOC_GPIO_1_REGS, but, GPIO_INTC_TYPE_FALL_EDGE);
    // gpioIntTypeSet(SOC_GPIO_1_REGS, but2, GPIO_INTC_TYPE_FALL_EDGE);
    // gpioIntTypeSet(SOC_GPIO_1_REGS, but3, GPIO_INTC_TYPE_FALL_EDGE);
	...
	while(TRUE){
		if(GPIOPinRead(getAddr(mod), but) == 0){
			ConsoleUtilsPrintf("\n<<< Botao 1 Pressionado >>>\n");
			ConsoleUtilsPrintf("\n<<< Minuto Adicionado >>>\n");
		...
		if(GPIOPinRead(getAddr(mod), but) == 0){
			...
			for(int i = 0; i < 10; i++){
				ConsoleUtilsPrintf("Reiniciando em %d\n",i);
				Delay(600);
			}
			HWREG(SOC_PRCM_REGS + 0xF00) |= 2;
		}
...
*END*-----------------------------------------------------------*/    
// static void gpioIsr(void) {
// 	if(GPIOPinRead(getAddr(mod), but) == 0) flagIsr = 1;

// 	if(GPIOPinRead(getAddr(mod), but2) == 0) flagIsr2 = 1;

// 	if(GPIOPinRead(getAddr(mod), but3) == 0) flagIsr3 = 1;
