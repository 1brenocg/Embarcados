...
*******************************************************************************/
   	...
   	unsigned int mod = 1, mod2 = 2, but = 31, but2 = 29, but3 = 30;
    ...
    ConsoleUtilsPrintf("\n\n\r ######################################\n\r");
	ConsoleUtilsPrintf("\r ########  GPIO INTERRUPT AND TIMER v.5  ## ######\n\r");
	...
	/* configure gpio interrupt on the INTC */
   	gpioAintcConf();
    	...
    	gpioIntTypeSet(SOC_GPIO_1_REGS, but3, GPIO_INTC_TYPE_BOTH_EDGE);
   	...
   	while(1){
			...
			ConsoleUtilsPrintf("Voce tem %d minutos",min);
...
static void gpioIsr(void) {
	...
	if(GPIOPinRead(getAddr(1), 31)) flagIsr = 1;

    else if(GPIOPinRead(getAddr(1), 29)) flagIsr2 = 1;

    else if(GPIOPinRead(getAddr(1), 30)) flagIsr3 = 1;	
